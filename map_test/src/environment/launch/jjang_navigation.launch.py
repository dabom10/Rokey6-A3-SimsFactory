...
def generate_launch_description():
    ...
    map_dir = LaunchConfiguration(
        ...
        default=os.path.join(
            get_package_share_directory("environment"), "maps", "jjang_map.yaml"
        ),
    )
    ...
    param_dir = LaunchConfiguration(
        "params_file",
        default=os.path.join(
            get_package_share_directory("environment"), "params", "jjang_params.yaml"
        ),
    )

    nav2_bringup_launch_dir = os.path.join(get_package_share_directory("nav2_bringup"), "launch")

    rviz_config_dir = os.path.join(get_package_share_directory("environment"), "rviz", "jjang_navigation.rviz")
