...
def generate_launch_description():
    ...
    ld_automatic_goal = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(pkg_dir, "launch", "jjang_navigation.launch.py"))
    )
    ...
    # # [🌟 추가됨] 라이다(base_scan)와 로봇 베이스(base_footprint)를 연결하는 정적 TF 퍼블리셔
    # static_tf_node = Node(
    #     package='tf2_ros',
    #     executable='static_transform_publisher',
    #     name='link_base_to_scan',
    #     # arguments: [x, y, z, yaw, pitch, roll, parent_frame, child_frame]
    #     # 라이다가 로봇 중심에서 살짝 위(z=0.2m)에 있다고 가정하고 연결합니다.
